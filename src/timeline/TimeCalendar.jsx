...
const BodyAxisCell = styled.div`
  ...
  left: 0;
  z-index: 6;
  background: ${(p) => p.$gridBg || 'transparent'}, ${(p) => p.$subBgColor || '#fafafa'};  /* 子区域背景色（时间轴/表头/锚定区） */
...
export default function TimeCalendar({
  ...
  columnHeader = "时间",
  rowHeader = "人员",
  timeFormat = "single",  // 时间格式�?single' (08:00) �?'range' (08:00-08:30)
  selectedEventId = null,  // 选中的事�?ID
  // 外观配置
  backgroundColor = "",
  subBackgroundColor = "",
  mainFontSize = 0,
  ...
  const getEventPosition = (ev) => {
    // 直接使用已标准化�?startTime/endTime（纯时间字符串）
    const [sh, sm] = begintime.split(":").map(Number);
        ...
        <BodyRow style={{ height: calculatedRowHeight }} $totalWidth={totalWidth}>
          ...
          {resources.length === 0 && (
            ...
            >
              ...
              <div>暂无人员数据</div>
              <div style={{ marginTop: '4px', fontSize: '12px' }}>����������������Դ</div>
            </div>
          )}
          
          <BodyAxisCell axisWidth={axisWidth} $subBgColor={subBackgroundColor} $gridBg={gridBg} style={{ height: calculatedRowHeight }}>
            <TimelineAxis 
              begintime={begintime}
              endtime={endtime}
          ...
          {resources.map((s, idx) => {
            ...
            var eventLayouts = staffEvents.map(function(ev) {
              ...
              return mainLayouts[mi++] || { left: 0, width: 100, zIndex: Z_BAR_MAIN };
            });
            ...
            return (
              ...
              >
                ...
                {staffEvents.map((ev, evIdx) => {
                  ...
                  // 获取布局信息
                  const layout = eventLayouts[evIdx] || { left: 0, width: 100, zIndex: Z_BAR_MAIN };
