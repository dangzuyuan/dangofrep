...
export function useDragMove({
  ...
  const handleMouseDown = useCallback((e, event, resourceId) => {
    ...
    // [修改] stopPropagation removed - 在React合成事件中它会阻止capture阶段，导致onMouseDownCapture不触发
    ...
    // [修改] RAF延迟state更新，让click在isDragging变true前完成
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      if (previewStartMin === originalStartMin && (newResourceId === originalResourceId || !newResourceId)) {
        setIsDragging(false);
        setDragState(null);
...
export function useDragResize({
  ...
  const handleResizeStart = useCallback((e, event, resourceId, direction) => {
    ...
    // [修改] 移除 stopPropagation，React合成事件会阻止capture阶段
...
export function useDragCreate({
  ...
  const handleSelectStart = useCallback((e, resourceId) => {
    ...
    var startMin = pixelToMinutes(e.clientY - rect.top, rect.height, maxMinutes - minMinutes);
  ...
  useEffect(() => {
    const handleMouseMove = (moveEvent) => {
      ...
      if (!selectRef.current.originX) return; // 初始化未完成
      var dx = Math.abs(moveEvent.clientX - selectRef.current.originX);
      var dy = Math.abs(moveEvent.clientY - selectRef.current.originY);
      if (dx < 3 && dy < 3) return; // <3px → 忽略
      ...
      var startMin = Math.min(selectRef.current.startMin, currentMin);
      var endMin = Math.max(selectRef.current.startMin, currentMin);
    ...
    const handleMouseUp = () => {
      ...
      const { onSelectSlot, timejiange, begintime } = configRef.current;
      var tj = timejiange || 15;
      if (selectRef.current.startTime && Date.now() - selectRef.current.startTime < 200) { cleanup(); return; }
      var snappedStart = snapToGrid(selectRef.current.startMin, tj);
      var snappedEnd = snapToGrid(selectRef.current.endMin, tj);
      // 防止 endMin 异常跳跃：限制在 startMin ± 最大轴范围
      if (Math.abs(snappedEnd - snappedStart) > selectRef.current.totalMinutes) {
        snappedEnd = snappedStart + tj;
      }

      if (Math.abs(selectRef.current.endMin - selectRef.current.startMin) < tj) {
        cleanup();
        return;
      }
      ...
      var absStart = bMin + Math.min(snappedStart, snappedEnd);
      var absEnd = bMin + Math.max(snappedStart, snappedEnd);
      ...
      if (Math.abs(absEnd - absStart) < timejiange) {
        cleanup();
        return;
      }
